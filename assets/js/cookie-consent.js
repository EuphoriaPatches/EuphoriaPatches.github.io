(function () {
  ...
  const regionSettings = {
    eu: {
      ...
      bannerText:
        "We use cookies to enhance your browsing experience and analyze site traffic. By clicking 'Accept', you consent to our use of cookies. For more information, see our <a href='/privacy-policy'>Privacy Policy</a>.",
      requireConsent: true,
      showRejectAll: true,
    ...
    us: {
      ...
      bannerText:
        "We use cookies to enhance your experience and analyze site traffic. By continuing to use our site, you agree to our <a href='/privacy-policy'>Privacy Policy</a>.",
    ...
    ca: {
      bannerTitle: "We value your privacy",
      bannerText:
        "We use cookies to enhance your browsing experience and analyze site traffic. By clicking 'Accept', you consent to our use of cookies in accordance with CCPA requirements. For more information, see our <a href='/privacy-policy'>Privacy Policy</a>.",
      requireConsent: true,
      showRejectAll: true,
      expireDays: 365, // 1 year
    },
  ...
  function detectUserRegion() {
    ...
    if (timezone.includes("Europe/")) {
      ...
      console.log(
        "✓ DETECTED: Canada (by timezone - America/Toronto or Canada/)",
      );
      console.groupEnd();
      cachedRegion = "ca";
      return cachedRegion;
    ...
    console.log(
      "⚠ Timezone detection failed, falling back to language detection",
    );
    ...
    if (
      euLanguages.some((lang) => browserLang.startsWith(lang)) ||
      browserLang.startsWith("en-GB")
    ) {
      const matchedLang =
        euLanguages.find((lang) => browserLang.startsWith(lang)) || "en-GB";
    ...
    console.log(
      "⚠ FALLBACK: Defaulting to EU (most privacy-respecting option)",
    );
    console.log(
      `Unable to detect region reliably from timezone "${timezone}" or language "${browserLang}"`,
    );
    console.groupEnd();
    cachedRegion = "eu";
    return cachedRegion;
  ...
  function savePreferences(preferences) {
    ...
    expiryDate.setTime(
      expiryDate.getTime() +
        getRegionSettings().expireDays * 24 * 60 * 60 * 1000,
    );
  ...
  function initBanner() {
    ...
    console.groupCollapsed(
      `Cookie Consent - Region: ${cachedRegion}${wasCached ? " (cached)" : " (detected)"} ${consentStatus}`,
    );
    ...
    if (cookieConsent) {
      ...
      try {
        ...
        if (preferences.expires) {
          ...
          expiryDate.setTime(
            expiryDate.getTime() + settings.expireDays * 24 * 60 * 60 * 1000,
          );
        }
      } catch (e) {
    ...
    console.log(
      `Cookie will expire on: ${expiryDate.toUTCString()} \n(${daysRemaining} days remaining)`,
    );
